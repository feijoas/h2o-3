...
function cleanupTshark () {
  kill -9 ${PID_4} 1> /dev/null 2>&1
}
...
java -Dai.h2o.name=$CLUSTER_NAME -ea \
    ...
    water.network.SSLEncryptionTest "gbm" > $OUTDIR/nonSSL.txt

cleanupTshark
cleanup

startCluster
echo Running nonSSL DL test...

java -Dai.h2o.name=$CLUSTER_NAME -ea \
    -cp "build/libs/h2o-algos-test.jar${SEP}build/libs/h2o-algos.jar${SEP}../h2o-core/build/libs/h2o-core.jar${SEP}../h2o-core/build/libs/h2o-core-test.jar${SEP}../h2o-genmodel/build/libs/h2o-genmodel.jar${SEP}../lib/*" \
    water.network.SSLEncryptionTest "dl" >> $OUTDIR/nonSSL.txt
...
echo Running SSL GBM test...

tshark -i ${INTERFACE} -T fields -e data -w ${OUTDIR}/h2o-SSL.pcap 1> /dev/null 2>&1 & PID_4=$!

java -Dai.h2o.name=$CLUSTER_NAME -ea \
    -cp "build/libs/h2o-algos-test.jar${SEP}build/libs/h2o-algos.jar${SEP}../h2o-core/build/libs/h2o-core.jar${SEP}../h2o-core/build/libs/h2o-core-test.jar${SEP}../h2o-genmodel/build/libs/h2o-genmodel.jar${SEP}../lib/*" \
    water.network.SSLEncryptionTest "gbm" src/test/resources/ssl.properties > $OUTDIR/SSL.txt

cleanupTshark
cleanup

startCluster
echo Running SSL DL test...

java -Dai.h2o.name=$CLUSTER_NAME -ea \
    -cp "build/libs/h2o-algos-test.jar${SEP}build/libs/h2o-algos.jar${SEP}../h2o-core/build/libs/h2o-core.jar${SEP}../h2o-core/build/libs/h2o-core-test.jar${SEP}../h2o-genmodel/build/libs/h2o-genmodel.jar${SEP}../lib/*" \
    water.network.SSLEncryptionTest "dl" src/test/resources/ssl.properties >> $OUTDIR/SSL.txt
